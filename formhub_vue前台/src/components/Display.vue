  ...
  <div class="display"  v-loading="loading" element-loading-text="加载中...">
    ...
    <div class="content">
      ...
      <el-card class="box-card" v-for="(item,index) in detail.questions" :key="index">
        ...
        <!--单选题展示-->
        <div class="text item"  :key="index" v-for="(optionItem, index) in item.options">
        ...
        <el-checkbox-group v-if="item.qtype=='2'" v-model="item.checkboxValue">
          <div class="text item" :key="index" v-for="(optionItem, index) in item.options">
            <el-checkbox :label="optionItem.id" :key="optionItem.id" @change="checkReflash" style="margin: 5px;">{{ optionItem.title }}</el-checkbox>
          </div>
        </el-checkbox-group>
      ...
      <div class="bottom">
        ...
        <el-link type="info"><router-link to="/index">formhub&nbsp;提供技术支持</router-link></el-link>
      </div>
    </div>
  </div>
</template>
       ...
       submit(){
        ...
        this.detail.questions.forEach(async item=>{
          ...
          if(item.qtype=='1' && !item.radioValue) {
           ...
           if(item.qtype=='2' && (item.checkboxValue.length==0 || !item.checkboxValue)) {
        ...
        uploadW({
         ...
         'options': wjoptions
        },wjId, this.$cookieStore.getCookie('formhub'))
          .then(data=>{
            //console.log(data);
            if(data.code==200){
              //提交成功
